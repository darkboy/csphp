 ...
 * Csphp::response()->setHeader($httpCodeNum);
 * Csphp::response()->setHttpCode($httpCodeNum);
 * Csphp::response()->setContentType($typeOrName);
 *
 * Csphp::response()->setCookie($cookieName,$v, $ttl=0, $path='/',$domain=null, $secure=false, $httpOnly=false);
 * Csphp::response()->delCookie($ckNameOrNameArr);
 *
 * Csphp::response()->redirect($url,$type);
 *
 * Csphp::response()->setBodyStr($str, $isAppend=false);
 * Csphp::response()->send();
 * Csphp::response()->sendP3P();
